    ...
    <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
      ...
      <div class="flex flex-col flex-1 min-h-screen overflow-x-hidden overflow-y-auto">
            ...
            <nav aria-label="Secondary" class="hidden space-x-2 md:flex md:items-center">
              ...
              <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-ref="userMenu" x-transition:enter="transition-all transform ease-out" x-transition:enter-start="translate-y-1/2 opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition-all transform ease-in" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-1/2 opacity-0" @click.away="open = false" @keydown.escape="open = false" class="absolute right-0 w-48 py-1 bg-white rounded-md shadow-lg top-12 ring-1 ring-black ring-opacity-5 dark:bg-dark focus:outline-none" tabindex="-1" role="menu" aria-orientation="vertical" aria-label="User menu">
                  ...
                  <a href="../pages/home.php" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                    Home
                  </a>
                  ...
                  <a href="../pages/about.php" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                    About
                  </a>
                  <a href="../pages/signin.php?logout=true" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                    Logout
                  </a>
                </div>
              </div>
        ...
        if (!empty($_SERVER["REQUEST_METHOD"] == "POST")) {
          ...
          if ($main_size && $image_size1 && $image_size2 && $image_size3 > 500000) {
            ...
            print_r($image_ex);
            echo "<br>";
            $allowed_exs = array('jpg', 'png', 'gif', 'jpeg');
            if (in_array($image_ex_lc, $allowed_exs)) {
              $new_img_name = $main_image;
              ...
              echo "<br>";
              move_uploaded_file($main_tmp, $img_upload_path);
            } else {
            }
            ////image_image1

            $image_ex1 = pathinfo($image_name1, PATHINFO_EXTENSION);
            $image_ex_lc1 = strtolower($image_ex1);
            $allowed_exs1 = array('jpg', 'png', 'gif', 'jpeg');
            if (in_array($image_ex_lc1, $allowed_exs1)) {
              $new_img_name1 = $image_ex_lc1;

              $img_upload_path1 = 'uploads/' . $new_img_name1;
              move_uploaded_file($image_temp1, $img_upload_path1);
            } else {
            }
            ////image_image2
            $image_ex2 = pathinfo($image_name2, PATHINFO_EXTENSION);
            $image_ex_lc2 = strtolower($image_ex2);
            $allowed_exs2 = array('jpg', 'png', 'gif', 'jpeg');
            if (in_array($image_ex_lc2, $allowed_exs2)) {
              $new_img_name2 = uniqid('IMG', true) . '.' . $image_ex_lc2;

              $img_upload_path2 = 'uploads/' . $new_img_name2;
              move_uploaded_file($image_temp2, $img_upload_path2);
            } else {
            }
            ////image_image3
            $image_ex3 = pathinfo($image_name3, PATHINFO_EXTENSION);
            $image_ex_lc3 = strtolower($image_ex3);
            $allowed_exs3 = array('jpg', 'png', 'gif', 'jpeg');
            if (in_array($image_ex_lc3, $allowed_exs3)) {
              $new_img_name3 = uniqid('IMG', true) . '.' . $image_ex_lc3;
              $img_upload_path3 = 'uploads/' . $new_img_name3;

              move_uploaded_file($image_temp3, $img_upload_path3);
            } else {
            }
          }
...
price,status,main_picture,picture1,picture2,picture3,product_color,specification,capacity) VALUES (:category_id,:product_name,:discription,:price,:status,:main_picture,:picture1,:picture2,:picture3,:product_color,:specification,:capacity)";
      ...
      here;
              ...
              <input type="submit" class="btn btn-primary w-full" value="Add">
