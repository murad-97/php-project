...
<body>
    ...
    <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
      ...
      <div class="flex-1 h-full overflow-x-hidden overflow-y-auto">
            ...
            <div class="grid grid-cols-1 gap-8 p-4 lg:grid-cols-2 xl:grid-cols-4">
              ...
              <div class="flex items-center justify-between p-4 bg-white rounded-md dark:bg-darker">
                <div>
                  ...
                  <span class="text-xl font-semibold">
                    <?php
                    $category = $conn->prepare("SELECT id FROM category");
                    $category->execute();
                    $result_of_category = $category->fetchAll();
                    $number = count($result_of_category);
                    if ($number > 0) {
                      echo $number;
                    } else {
                      echo "<span>No find Categories</span>";
                    } ?>
                  </span>
              ...
              <div class="flex items-center justify-between p-4 bg-white rounded-md dark:bg-darker">
                <div>
                  ...
                  <span class="text-xl font-semibold">
                    <?php
                    $user = $conn->prepare("SELECT id FROM users");
                    $user->execute();
                    $result_of_user = $user->fetchAll();
                    $number_user = count($result_of_user);
                    if ($number_user > 0) {
                      echo $number_user;
                    } else {
                      echo "<span>No find Users</span>";
                    }
                    ?>
              ...
              <div class="flex items-center justify-between p-4 bg-white rounded-md dark:bg-darker">
                <div>
                  ...
                  <span class="text-xl font-semibold">
                    <?php
                    $order_detail = $conn->prepare("SELECT id FROM order_detail");
                    $order_detail->execute();
                    $result_of_order_detail = $order_detail->fetchAll();
                    $number_order_detail = count($result_of_order_detail);
                    if ($number_order_detail > 0) {
                      echo $number_order_detail;
                    } else {
                      echo "<span>No find orders</span>";
                    }
                    ?>
              ...
              <div class="flex items-center justify-between p-4 bg-white rounded-md dark:bg-darker">
                <div>
                  ...
                  <span class="text-xl font-semibold">
                    <?php
                    $prodcut = $conn->prepare("SELECT id FROM product");
                    $prodcut->execute();
                    $result_of_prodcut = $prodcut->fetchAll();
                    $number_prodcut = count($result_of_prodcut);
                    if ($number_prodcut > 0) {
                      echo $number_prodcut;
                    } else {
                      echo "<span>No find prodcut</span>";
                    }
                    ?>
                  </span>
                  <span class="inline-block px-2 py-px ml-2 text-xs text-green-500 bg-green-100 rounded-md">
        ...
        <footer class="flex items-center justify-between p-4 bg-white border-t dark:bg-darker dark:border-primary-darker">
          <div>Mobile-Tech &copy; 2023</div>
          <div>
            Made by
            <a href="#" target="" class="">Group Three</a>
          </div>
        </footer>
      </div>
  ...
  <script src="/Admin/build/js/script.js"></script>
  <script>
    const setup = () => {
      const getTheme = () => {
